...
enum TextureType {
    ...
    bloom, deferPos, deferNormal, deferColor, aoMap, aoNoise
};
...
public:
    ...
    Texture(unsigned int ID, TextureType type, bool use = true): ID(ID), type(type), shouldUse(use) {};
...
public:
    RepeatingTexture(int w, int h, const void *data, TextureType type = aoNoise);
};
