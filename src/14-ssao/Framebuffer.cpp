...
NormalFramebuffer::NormalFramebuffer(int w, int h, bool hdr, bool useStencil, const int *channelCount) {
    this->w = w;
    this->h = h;
    usingHdr = hdr;
    ...
    glBindTexture(GL_TEXTURE_2D, texId);
    int channels = channelCount ? *channelCount : 3;
...
MRTNormalBuffer::MRTNormalBuffer(int w, int h, int targetCount, bool hdr, const int *targetChannelCount, int stencil):
NormalFramebuffer(w, h, hdr, stencil, targetChannelCount)
{
