...
int main() {
    ...
    camera = new Camera();
    camera->position = vec3(0.f, 12.f, 10.f);
    camera->front = vec3(0.f, 0.f, -1.f);
    camera->upVec = vec3(0.f, 1.f, 0.f);
    camera->fov = 45;
    renderer->camera = camera;
  
    auto mat1 = scale(identityM, vec3(1, 1, 1));

    Scene scene;
    std::string modelDir = MODEL_DIR;
    scene.addModel(modelDir + "/nanosuit/nanosuit.obj");

    //    scene.addModel(modelDir + "/bunny/bunny.obj");
//    mat1 = scale(identityM, vec3(10, 10, 10));
    
//    scene.addModel(modelDir + "/rock/rock.obj");
    
    
    // 灯的位置
    auto mat2 = translate(identityM, vec3(0.5f, 15.f, 1.7f));
